...
class PelangganRepository:
    ...
    def create(self, db: DuckDBPyConnection, req: PelangganPostRequest):
        try:
            query = "INSERT INTO pelanggan (nama, alamat, no_hp, paket_id) VALUES (?, ?, ?, ?)"
            params = (req.nama, req.alamat, req.no_hp, req.paket_id)
    ...
    def update(self, db: DuckDBPyConnection, id: int, req: PelangganPostRequest):
        try:
            ...
            query = (
                "UPDATE pelanggan SET nama=?, alamat=?, no_hp=?, paket_id=? WHERE id=?"
            )
            params = (req.nama, req.alamat, req.no_hp, req.paket_id, id)
            db.execute(query, params)
            db.commit()
        except HTTPException as e:
            raise e
        except Exception as e:
            LOGGER.error(f"Error updating pelanggan: {e}")
            db.rollback()
            raise HTTPException(status_code=500, detail="Internal Server Error")
    ...
    def delete(self, db: DuckDBPyConnection, id: int):
        try:
            ...
            LOGGER.error(f"Error deleting pelanggan: {e}")
            db.rollback()
            raise HTTPException(status_code=500, detail="Internal Server Error")
